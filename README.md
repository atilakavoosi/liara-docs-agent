...
- سنجیده شده، نه فقط ادعا: `npx tsx scripts/eval.ts` روی ۳۴ سؤال طلایی
  ...
  بهتر می‌کنه). هر ۵ باخت یک الگوی مشترکن: کورپوس صفحه‌های هم‌نام رو در چند
  سرویس مختلف تکرار می‌کنه (مثلاً «تغییر پلن» هم زیر PaaS هست هم IaaS هم
  ایمیل‌سرور)، و بدون گفتن اسم سرویس، بازیابی گاهی نسخه‌ی اشتباه رو بالاتر
  می‌بره.
